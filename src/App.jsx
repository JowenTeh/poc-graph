...
export default function App() {
  ...
  useEffect(() => {
    const sampleQuery = 'Unlicensed manufacture and hawking of curry puffs at Blk 63B, Lengkok Bahru which involves Robiah';
  ...
  return (
    <>
      <div className="w-screen h-screen">
        ...
        {query && node && graph.hasNode(node) && <NodeInfo className={`absolute ${query ? 'top-36' : 'top-24'} left-6 w-2/6`} node={{degree: graph.degree(node), ...graph.getNodeAttributes(node)}} />}
      </div>
    </>
  )
}
